...
static void *dscr_default_random_thread(void *in)
{
	...
	for (unsigned long i = 0; i < COUNT; i++) {
		...
		FAIL_IF_EXIT(pthread_rwlock_rdlock(args->rw_lock));
		d = *args->expected_system_dscr;
		cur_dscr = get_dscr();
		cur_dscr_usr = get_dscr_usr();
		FAIL_IF_EXIT(pthread_rwlock_unlock(args->rw_lock));

		FAIL_IF_EXIT(cur_dscr != d);
		FAIL_IF_EXIT(cur_dscr_usr != d);

		if (rand() % 10 == 0) {
			unsigned long next_dscr;

			FAIL_IF_EXIT(pthread_rwlock_wrlock(args->rw_lock));
			next_dscr = (*args->expected_system_dscr + 1) % DSCR_MAX;
			set_default_dscr(next_dscr);
			*args->expected_system_dscr = next_dscr;
			FAIL_IF_EXIT(pthread_rwlock_unlock(args->rw_lock));
		}
	}
...
int dscr_default_random_test(void)
{
	...
	FAIL_IF(pthread_rwlockattr_setkind_np(
	...
	for (int i = 0; i < THREADS; i++) {
		...
		FAIL_IF(pthread_create(&threads[i].thread_id, NULL, dscr_default_random_thread, (void *)&threads[i]));
	}
