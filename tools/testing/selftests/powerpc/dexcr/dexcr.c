...
bool pr_get_dexcr_aspect(unsigned long which)
{
	return pr_get_dexcr(which) & PR_PPC_DEXCR_CTRL_SET;
}

bool pr_get_dexcr_aspect_onexec(unsigned long which)
{
	return pr_get_dexcr(which) & PR_PPC_DEXCR_CTRL_SET_ONEXEC;
}

bool pr_check_dexcr_aspect(unsigned long which)
{
	unsigned long dexcr = get_dexcr(EFFECTIVE);
	unsigned long aspect = pr_which_to_aspect(which);

	return (dexcr & aspect) == aspect;
}

bool pr_dexcr_aspect_supported(unsigned long which)
{
	return pr_get_dexcr(which) != ENODEV;
}
